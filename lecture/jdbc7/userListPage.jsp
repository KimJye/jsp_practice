...
<%
	...
	if(od==null)od="0";
	List<User> list = UserDAO.findAll(currentPage, pageSize, st, od);
...
<a id="createButton" class="btn btn-primary pull-right" 
	href="userCreate.jsp?pg=<%=currentPage%>&st=<%=stEncoded%>&od=<%=od%>">
...
<table class="table table-bordered table-condensed">
	...
	<tbody>
		<% for(User user : list){ %>
			<tr data-url="userListEdit.jsp?id=<%=user.getId() %>&pg=<%=currentPage%>&st=<%=stEncoded%>&od=<%=od%>">
				<td><%=user.getId() %></td>
