...
<%@ page import="java.util.List, java.util.Arrays, jdbc1.User, jdbc3.UserListPage" %>
...
<%
    	...
    	int recordCount = UserListPage.count();
...
if(click==null){
	...
	list=UserListPage.findAll(currentPage,pageSize);
}
else if("동의버튼".equals(click)){
	
	list=UserListPage.findByable(able,currentPage,pageSize);
	
}
else if("유형버튼".equals(click)){
	
	list=UserListPage.findByUserType(userType,currentPage,pageSize);
...
<title>라디오버튼,체크박스로 조회하기 구현페이징</title>
</head>
<body>
