...
public class StudentDAO {
	   ...
	   public static List<Student> findAll(int currentPage, int pageSize) throws Exception {//리턴타입이 List 타입인데 이렇게 가급적 부모타입으로 적어주는것이 좋다. 이유는 다형성때문. 다형성 구현할때는  1. 부모클래스나 인터페이스가  있어야 한다. 2. 자식이 부모 메소드를 재정의(오버라이딩)해야한다. 3. 부모타입의 변수로 메소드를 호출해야 다형성 호출이다.
		   String sql = "SELECT s.*, d.departmentName " +
		   "FROM student s LEFT JOIN department d ON s.departmentId = d.id LIMIT ?,?";
