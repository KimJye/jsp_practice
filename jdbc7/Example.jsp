<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%@ page import ="java.util.*, java.net.*, jdbc7.*, lecture1.ParseUtils" %>
...
<%
	...
	int recordCount = UserDAO.count(st);
	...
	if(click==null){
		...
		list=UserDAO.findAll(currentPage,pageSize, st, od);
	}
	else if("이름버튼".equals(click)){
		
		list=UserDAO.findByName(currentPage,pageSize ,st, od);
	}
	else if("학과버튼".equals(click)){
		
	
		if(de==0)list=UserDAO.findAll(currentPage,pageSize, st, od);
		else list=UserDAO.findByDepartmentId(currentPage,pageSize, de, od);
...
<a id="createButton" class="btn btn-primary pull-right" 
	href="userCreate.jsp?pg=<%=currentPage%>&st=<%=stEncoded%>&de=<%=de %>&od=<%=od%>">
	...
	<div class="form-group">
	 ...
	 <div class="form-group">
    	...
    	<select name="departmentId" class="form-control">
     	 <option value="0" <%= de == 0 ? "selected" : "" %>>전체</option>
     	 <% for (Department d : DepartmentDAO.findAll()) { %>      
      	  <option value="<%= d.getId() %>" <%= de == d.getId() ? "selected" : "" %>>
...
<table class="table table-bordered table-condensed">
	...
	<tbody>
		<% for(User user : list){ %>
			<tr data-url="userListEdit.jsp?id=<%=user.getId() %>&pg=<%=currentPage%>&st=<%=stEncoded%>&de=<%=de %>&od=<%=od%>">
				<td><%=user.getId() %></td>
