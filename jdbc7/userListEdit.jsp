...
<%
	...
	if(request.getMethod().equals("GET")){
		...
		if(s1==null || s1.length()==0)
			...
			UserDAO.update(user);
			response.sendRedirect("Example.jsp?pg="+pg+"&st="+srchTextEncoded+"&de="+de+"&od="+od);
			return;
		}
	}
...
<form method="post">
	...
	<a href="userDelete.jsp?id=<%=id %>&pg=<%=pg %>&st=<%=srchTextEncoded%>&de<%=de %>&od=<%=od %>"
		class="btn btn-default" onclick="return confirm('삭제하시겠습니까?')">
		<i class="glyphicon glyphicon-trash"></i>삭제
	</a>
	<a href="Example.jsp?pg=<%=pg %>&st=<%=srchTextEncoded %>&de<%=de %>&od=<%=od %>"
